...
public class ClientePage extends Navigation {
  ...
  private void cercaRistoranti() {
    ...
    scriviMessaggio("Nazione: %s\nCittà: %s\nIndirizzo: %s\n".formatted(nazione, citta, indirizzo));
    scriviMessaggio("==============================");
    ...
    try {
      ...
      if (input.equals("1")) {
        ...
        if (conferma("Sicuro di voler rimuovere dai preferiti?")) {
          rimuoviPreferito(ristoranteSelezionato.getId());
  ...
  public void aggiungiPreferito(Ristorante ristorante) {
    if (!cliente.getRistorantiIDs().contains(ristorante.getId())) {
      cliente.getRistorantiIDs().add(ristorante.getId());
      aggiornaUtente();
      scriviMessaggio("Aggiunto ai preferiti!");
    } else {
      scriviInfo("Questo ristorante è già nei tuoi preferiti.");
    }
  }
   ...
   */
  private void rimuoviPreferito(int ristoranteID) {
